...
class aggregation(nn.Module):
    ...
    def __init__(self, channel, n_classes):
        ...
        self.conv5 = nn.Conv((4 * channel), n_classes, 1)
    ...
    def execute(self, x1, x2, x3, x4):
        ...
        x4_1 = jt.contrib.concat((x4, self.conv_upsample6(self.upsample(x3_2))), dim=1)
        x4_1 = self.conv_concat4(x4_1)
        x = self.conv4(x4_1)
        x = self.conv5(x)
        # x = self.conv4(x3_2)
        # x = self.conv5(x)
...
class HarDMSEG_alter(nn.Module):

    def __init__(self, channel=32, n_classes=2):
        super(HarDMSEG_alter, self).__init__()
        self.relu = nn.ReLU()
    ...
    def execute(self, x):
        ...
        lateral_map_5 = nn.interpolate(ra5_feat, scale_factor=4, mode='bilinear')
