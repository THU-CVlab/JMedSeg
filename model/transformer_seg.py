...
class Encoder2D(nn.Module):
    ...
    def execute(self, x):
        ...
        x = x.reshape(b,c,h//p1,p1,w//p2,p2).transpose(0,2,4,3,5,1).reshape(b,-1,p1*p2*c)
        ...
        x = x.reshape(b,h,w,p1,p2,c).transpose(0,5,1,3,2,4).reshape(b,c,h*p1,w*p2)
        return (encode_x, x)
...
class Decoder2D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, features=[512, 256, 128, 64]):
        super().__init__()
        self.decoder_1 = nn.Sequential(nn.Conv(in_channels, features[0], 3, padding=1), nn.BatchNorm(features[0]), nn.ReLU(), nn.Upsample(scale_factor=2, mode='bilinear'))
        self.decoder_2 = nn.Sequential(nn.Conv(features[0], features[1], 3, padding=1), nn.BatchNorm(features[1]), nn.ReLU(), nn.Upsample(scale_factor=2, mode='bilinear'))
        self.decoder_3 = nn.Sequential(nn.Conv(features[1], features[2], 3, padding=1), nn.BatchNorm(features[2]), nn.ReLU(), nn.Upsample(scale_factor=2, mode='bilinear'))
        self.decoder_4 = nn.Sequential(nn.Conv(features[2], features[3], 3, padding=1), nn.BatchNorm(features[3]), nn.ReLU(), nn.Upsample(scale_factor=2, mode='bilinear'))
        self.final_out = nn.Conv(features[(- 1)], out_channels, 3, padding=1)
